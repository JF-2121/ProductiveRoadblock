...
class DRV2605:
    ...
    """
    __slots__ = ['_device', '_address', '_sequence']
    ...
    def __init__(self, i2c: I2C, address: int = DRV2605_ADDR) -> None:
        self._device = i2c
        ...
        self._write_u8(DRV2605_REG_MODE, 0x00)  # out of standby
        self._write_u8(DRV2605_REG_RTPIN, 0x00)  # no real-time-playback
        self._write_u8(DRV2605_REG_WAVESEQ1, 1)  # strong click
        self._write_u8(DRV2605_REG_WAVESEQ2, 0)
        self._write_u8(DRV2605_REG_OVERDRIVE, 0)  # no overdrive
        self._write_u8(DRV2605_REG_SUSTAINPOS, 0)
        self._write_u8(DRV2605_REG_SUSTAINNEG, 0)
        self._write_u8(DRV2605_REG_BREAK, 0)
        self._write_u8(DRV2605_REG_AUDIOMAX, 0x64)
        # Set ERM open-loop mode.
        self.use_ERM()
        # turn on ERM_OPEN_LOOP
        control3 = self._read_u8(DRV2605_REG_CONTROL3)
        self._write_u8(DRV2605_REG_CONTROL3, control3 | 0x20)
    ...
    def library(self) -> int:
        ...
        library.
        """
        return self._read_u8(DRV2605_REG_LIBRARY) & 0x07

    @library.setter
    def library(self, val: int) -> None:
        if not 0 <= val <= 6:
            raise ValueError("Library must be a value within 0-6!")
        self._write_u8(DRV2605_REG_LIBRARY, val)
    ...
    def realtime_value(self) -> int:
        ...
        """
        return self._read_u8(DRV2605_REG_RTPIN)
    ...
    def realtime_value(self, val: int) -> None:
        ...
        self._write_u8(DRV2605_REG_RTPIN, val)
    ...
    def set_waveform(self, effect_id: int, slot: int = 0) -> None:
        ...
        self._write_u8(DRV2605_REG_WAVESEQ1 + slot, effect_id)

    # pylint: disable=invalid-name
    def use_ERM(self) -> None:
        """Use an eccentric rotating mass motor (the default)."""
        feedback = self._read_u8(DRV2605_REG_FEEDBACK)
        self._write_u8(DRV2605_REG_FEEDBACK, feedback & 0x7F)

    # pylint: disable=invalid-name
    def use_LRA(self) -> None:
        """Use a linear resonance actuator motor."""
        feedback = self._read_u8(DRV2605_REG_FEEDBACK)
        self._write_u8(DRV2605_REG_FEEDBACK, feedback | 0x80)
...
class _DRV2605_Sequence:
    ...
    def __setitem__(self, slot: int, effect) -> None:
        ...
        self._drv2605._write_u8(DRV2605_REG_WAVESEQ1 + slot, effect.raw_value)
    ...
    def __getitem__(self, slot: int):
        ...
        if not 0 <= slot <= 7:
            raise IndexError("Slot must be a value within 0-7!")
        # pylint: disable=protected-access
        slot_contents = self._drv2605._read_u8(DRV2605_REG_WAVESEQ1 + slot)
